#!/usr/bin/env python3
"""
Analyze whether changes in NY Fed survey medians predict later SEP median changes.

For each NY Fed survey observation, the future SEP is the first SEP released on
or after the survey received-by date. The SEP change is measured from the latest
prior SEP median to that future SEP median.
...
Outputs:
  - data_out/nyfed_future_sep_predictive_analysis.csv
  - data_out/nyfed_future_sep_predictive_event_analysis.csv
  - data_out/nyfed_future_sep_predictive_summary.csv
  - data_out/nyfed_future_sep_predictive_regressions.csv
  - data_out/nyfed_future_sep_predictive.png
"""
...
def read_inputs(nyfed_path: Path, sep_path: Path) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    return market, sep.sort_values("sep_date")
...
def build_survey_level_panel(market: pd.DataFrame, sep: pd.DataFrame) -> pd.DataFrame:
    rows = market.sort_values("received_by_date")
    prior = pd.merge_asof(
        rows,
        sep,
    ...
    ).rename(columns={"sep_date": "prior_sep_date", "sep_median": "prior_sep_median"})
    future = pd.merge_asof(
        prior.sort_values("received_by_date"),
        sep,
        left_on="received_by_date",
        right_on="sep_date",
        direction="forward",
        allow_exact_matches=True,
    ).rename(columns={"sep_date": "future_sep_date", "sep_median": "future_sep_median"})
    ...
    future["sep_change_pp"] = future["future_sep_median"] - future["prior_sep_median"]
    future["sep_change_bps"] = (future["sep_change_pp"] * 100).round(1)
    future["days_to_future_sep"] = (
        future["future_sep_date"] - future["received_by_date"]
    ).dt.days
    future["days_since_prior_sep"] = (
        future["received_by_date"] - future["prior_sep_date"]
    ).dt.days
    ...
    output_columns = [
        ...
        "sep_change_pp",
        "sep_change_bps",
        "days_since_prior_sep",
        "days_to_future_sep",
        "source",
        "local_path",
        "receipt_source_file",
    ]
    return future[output_columns].sort_values(["received_by_date", "panel"])


def build_event_level_panel(market: pd.DataFrame, sep: pd.DataFrame) -> pd.DataFrame:
    """Use the latest survey received in each SEP window for each panel."""
    rows: List[Dict] = []
    market = market.dropna(subset=["survey_change_bps"]).copy()

    for _, sep_row in sep.iloc[1:].iterrows():
        future_sep_date = sep_row["sep_date"]
        prior_sep = sep[sep["sep_date"] < future_sep_date].iloc[-1]
        available = market[
            (market["received_by_date"] > prior_sep["sep_date"])
            & (market["received_by_date"] <= future_sep_date)
        ].copy()
        if available.empty:
            continue

        for panel, group in available.groupby("panel"):
            latest = group.sort_values(["received_by_date", "survey_date"]).iloc[-1]
            sep_change_pp = sep_row["sep_median"] - prior_sep["sep_median"]
            rows.append({
                "future_sep_date": future_sep_date,
                "prior_sep_date": prior_sep["sep_date"],
                "panel": panel,
                "survey_date": latest["survey_date"],
                "received_by_date": latest["received_by_date"],
                "market_median": latest["pctl50"],
                "previous_market_median": latest["previous_market_median"],
                "survey_change_pp": latest["survey_change_pp"],
                "survey_change_bps": latest["survey_change_bps"],
                "prior_sep_median": prior_sep["sep_median"],
                "future_sep_median": sep_row["sep_median"],
                "sep_change_pp": sep_change_pp,
                "sep_change_bps": round(sep_change_pp * 100, 1),
                "days_to_future_sep": (future_sep_date - latest["received_by_date"]).days,
                "days_since_prior_sep": (latest["received_by_date"] - prior_sep["sep_date"]).days,
            })

    return pd.DataFrame(rows).sort_values(["future_sep_date", "panel"])
...
def summarize(panel: pd.DataFrame, sample_type: str) -> pd.DataFrame:
    ...
    for sample, group in groups:
        ...
        rows.append({
            ...
            "corr_survey_change_sep_change": round(
                group[["survey_change_bps", "sep_change_bps"]].corr().iloc[0, 1],
                3,
            ),
            "r_squared": round(r_squared, 3),
            "intercept_bps": round(coefficients.get("intercept", np.nan), 2),
            "slope_sep_bps_per_survey_bp": round(
                coefficients.get("survey_change_bps", np.nan),
                3,
            ),
            ...
            **{
            ...
            "avg_days_to_future_sep": round(group["days_to_future_sep"].mean(), 1),
        })
...
def build_regressions(
    ...
    for sample_type, panel in [
        ...
        ("event_level_latest_survey", event_panel),
    ]:
        groups: List[Tuple[str, pd.DataFrame]] = [("All", panel)]
        groups.extend((panel_name, panel[panel["panel"] == panel_name]) for panel_name in PANEL_ORDER)
        for sample, group in groups:
            ...
            rows.append({
                "sample_type": sample_type,
                "sample": sample,
                "model": "future_sep_change_bps_on_survey_change_bps",
                "n": n_obs,
                "r_squared": round(r_squared, 3),
                "intercept": round(coefficients.get("intercept", np.nan), 4),
                "survey_change_bps": round(coefficients.get("survey_change_bps", np.nan), 4),
            })
        ...
        coefficients, r_squared, n_obs = ols(
            panel_fe["sep_change_bps"],
            panel_fe[["survey_change_bps", "panel_fe_combined", "panel_fe_smp"]],
        )
        rows.append({
            "sample_type": sample_type,
            "sample": "All",
            "model": "future_sep_change_bps_on_survey_change_bps_panel_fe",
            "n": n_obs,
            "r_squared": round(r_squared, 3),
            "intercept": round(coefficients.get("intercept", np.nan), 4),
            "survey_change_bps": round(coefficients.get("survey_change_bps", np.nan), 4),
...
def plot_predictive_analysis(
    ...
    configs = [
        ("All Survey Observations", survey_panel),
        ("Latest Survey Before Each SEP", event_panel),
    ]
    ...
    for ax, (title, panel) in zip(axes, configs):
        ...
        slope = coefficients.get("survey_change_bps", np.nan)
        intercept = coefficients.get("intercept", np.nan)
        if pd.notna(slope) and pd.notna(intercept):
            x_values = np.linspace(
                panel["survey_change_bps"].min(),
                panel["survey_change_bps"].max(),
                100,
            )
            ax.plot(
                x_values,
        ...
        ax.set_xlabel("NY Fed Survey Median Change (bps)", fontweight="bold")
        ax.legend(loc="upper left", framealpha=0.95, fontsize=9)

    axes[0].set_ylabel("Next SEP Median Change (bps)", fontweight="bold")
    fig.suptitle(
        "Do Changes in NY Fed Survey Medians Predict Future SEP Median Changes?",
        fontsize=16,
        fontweight="bold",
        y=0.97,
    )
    add_chart_footer(
        ...
        "Future SEP is first SEP released on or after survey received-by date.",
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Analyze whether NY Fed survey median changes predict later SEP median changes."
    )
    parser.add_argument(
        "--nyfed-csv",
    ...
    parser.add_argument(
        "--output-csv",
        default="data_out/nyfed_future_sep_predictive_analysis.csv",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        help="Event-level latest-survey-before-SEP output CSV.",
    )
    parser.add_argument(
        "--summary-csv",
        default="data_out/nyfed_future_sep_predictive_summary.csv",
        type=Path,
        help="Predictive summary output CSV.",
    )
    parser.add_argument(
        "--regression-csv",
        default="data_out/nyfed_future_sep_predictive_regressions.csv",
        type=Path,
        help="Regression output CSV.",
    )
    parser.add_argument(
        "--output-png",
        default="data_out/nyfed_future_sep_predictive.png",
        type=Path,
    ...
    summary = pd.concat(
        [
            ...
            summarize(event_panel, "event_level_latest_survey"),
        ],
        ignore_index=True,
    )
    ...
    event_all = summary[
        (summary["sample_type"] == "event_level_latest_survey") & (summary["sample"] == "All")
    ].iloc[0]
    ...
    print(
        ...
        f"slope={survey_all['slope_sep_bps_per_survey_bp']:.3f}; "
        f"event-level R2={event_all['r_squared']:.3f}, "
        f"slope={event_all['slope_sep_bps_per_survey_bp']:.3f}"
    )
